...
from models import PowerState, HostState, HostsHealthStatusWrapper
...
def print_dashboard_dict(power_state: PowerState, hosts_status: dict[str, HostState]):
    clear_screen()
    print("=" * 60)
...
def print_dashboard(power_state: PowerState, hosts_status_mutex_wrapper: HostsHealthStatusWrapper):
    hosts_status = hosts_status_mutex_wrapper.get() # SNAPSHOT
    clear_screen()
    print("=" * 60)
    print(f"  POWER STATE: {power_state.status.name}")
    print("=" * 60)

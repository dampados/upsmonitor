import pytest
from models import PowerStateName, PowerState, Inputs, ActionBoxMock, Routine
...
class TestReact:
    def test_new_data_resets_counter_and_updates_values(self):
        ...
        # Default is None (empty threads) — no action called yet
        assert mock.current_routine is None
    ...
    def test_silence_below_threshold_increments_counter_no_action(self):
        ...
        assert new.status == PowerStateName.UNKNOWN
        # Still empty — no state change yet
        assert mock.current_routine is None
    ...
    def test_silence_at_threshold_triggers_state_change_and_calls_action(self):
        ...
        assert new.ticks_counter == 0
        assert mock.current_routine == Routine.RESTORE
    ...
    def test_silence_at_threshold_with_bad_on_bbu_calls_suspend(self):
        ...
        assert new.ticks_counter == 0
        assert mock.current_routine == Routine.SUSPEND

    @pytest.mark.parametrize("canary,switches,expected_status,expected_routine", [
        (False, False, PowerStateName.BAD_ON_BBU, Routine.SUSPEND),
        (False, True, PowerStateName.BAD_CANARY_DEAD, Routine.RESTORE),
        (True, True, PowerStateName.OK_HEALTHY, Routine.RESTORE),
        (True, False, PowerStateName.OK_GENERATOR, Routine.RESTORE),
    ])
    def test_all_state_mappings_trigger_correct_action(self, canary, switches, expected_status, expected_routine):
        ...
        assert new.ticks_counter == 0
        assert mock.current_routine == expected_routine
    ...
    def test_start_from_unknown_increments_counter(self):
        ...
        # Still empty — haven't reached threshold yet
        assert mock.current_routine is None
    ...
    def test_mixed_inputs_canary_only_updates_canary(self):
        ...
        assert new.ticks_counter == 0
        assert new.status == PowerStateName.UNKNOWN
        # No action called for mixed inputs
        assert mock.current_routine is None
    ...
    def test_silence_after_counter_reset_restarts_counting(self):
        ...
        assert new.ticks_counter == 1
        assert new.status == PowerStateName.UNKNOWN
        # Still empty — starting from zero
        assert mock.current_routine is None
    ...
    def test_repeated_silence_after_threshold_does_not_trigger_twice(self):
        ...
        assert new.ticks_counter == 0
        assert new.status == PowerStateName.OK_HEALTHY
        # Already in OK_HEALTHY, so no new action called
        assert mock.current_routine is None